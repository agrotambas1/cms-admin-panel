...
interface CaseSturyFromProps {
  form: UseFormReturn<CreateCaseStudyForm>;
  onSubmit: (data: CreateCaseStudyForm) => void;
  loading: boolean;
...
export function CaseStudyForm({
  ...
  const [outcomes, setOutcomes] = useState<CaseStudyOutcome[]>(
    form.getValues("outcomes") || [{ metric: "", value: "" }],
  );

  const addOutcome = () => {
    const currentOutcomes = form.getValues("outcomes") || [];
    form.setValue("outcomes", [...currentOutcomes, { metric: "", value: "" }]);
  };
  ...
  const moveOutcomeUp = (index: number) => {
    if (index === 0) return;
    const currentOutcomes = [...(form.getValues("outcomes") || [])];
    [currentOutcomes[index], currentOutcomes[index - 1]] = [
      currentOutcomes[index - 1],
      currentOutcomes[index],
    ];
  ...
  const moveOutcomeDown = (index: number) => {
    const currentOutcomes = [...(form.getValues("outcomes") || [])];
    if (index === currentOutcomes.length - 1) return;
    [currentOutcomes[index], currentOutcomes[index + 1]] = [
      currentOutcomes[index + 1],
      currentOutcomes[index],
    ];
  ...
  const handleFormSubmit = (data: CreateCaseStudyForm) => {
    if (publishedAt && data.status === "draft") {
      setShowWarning(true);
      return;
    }
  ...
  const handleConfirmStatusChange = () => {
    ...
    if (
      data.status === "in marketing review" ||
      data.status === "in technical review" ||
      data.status === "draft" ||
              ...
              <div className="lg:col-span-1 space-y-4">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select a publication to publish this case study
                    </p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                          ...
                          <FormItem>
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="in technical review">
                                  In Technical Review
                                </SelectItem>
                                <SelectItem value="in marketing review">
                                  In Marketing Review
                                </SelectItem>
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                            <div className="space-y-0.5">
                              ...
                              <FormDescription>
                                Mark this case study as featured
                              </FormDescription>
                            </div>
                            <FormControl>
                              <Switch
              ...
              <div className="lg:col-span-3 space-y-4">
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Describe the overview of the case study.
                    </p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Describe the problem of the case study.
                    </p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Describe the solution of the case study.
                    </p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Outcomes Description</FormLabel>
                            <FormControl>
                              <Editor
                                apiKey={process.env.NEXT_PUBLIC_TINYMCE_API_KEY}
                                value={field.value || ""}
                      ...
                      {form.watch("outcomes")?.map((_, index) => (
                        ...
                        >
                            ...
                            <div className="flex gap-1">
                              {index > 0 && (
                                <Button
                                  ...
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => moveOutcomeUp(index)}
                                >
                                  <ChevronUp className="h-4 w-4" />
                                </Button>
                              )}
                              {index <
                                (form.watch("outcomes")?.length || 0) - 1 && (
                                <Button
                                  type="button"
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => moveOutcomeDown(index)}
                                >
                          ...
                          <FormField
                            control={form.control}
                            name={`outcomes.${index}.metric`}
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Metric</FormLabel>
                                <FormControl>
                                  <Input
                                    placeholder="e.g., Revenue Growth, User Engagement, Cost Reduction"
                                    {...field}
                                    value={field.value ?? ""}
                                  />
                                </FormControl>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <FormControl>
                                  <Input
                                    placeholder="e.g., +150%, 2M users, $500K saved"
                                    {...field}
                                    value={field.value ?? ""}
                                  />
                                </FormControl>
                      ...
                      {(form.watch("outcomes")?.length || 0) < 10 && (
                        <Button
                          type="button"
                          variant="outline"
                          onClick={addOutcome}
                ...
                <div className="flex items-center justify-end gap-4">
                  <Button variant="ghost" disabled={loading}>
                    <Link href="/case-studies">Cancel</Link>
                  </Button>
                  <Button
                    type="submit"
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-none max-w-[95vw] w-[95vw] h-[90vh] p-6 flex flex-col overflow-hidden">
          <DialogHeader>
            ...
            <DialogDescription>
              Choose an image from media library to use as case study thumbnail.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showWarning} onOpenChange={setShowWarning}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This case study has already been published and may be indexed by
